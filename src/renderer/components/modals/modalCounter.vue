...
export default {
  ...
  methods: {
    getrelist(){
      ...
      if(!this.comlist.length){
        ipcRenderer.invoke(eventNames.portList).then((res)=>{
          if(res?.data){
            res?.data?.forEach((el)=>{
              if(el.path != 'COM1'){
    ...
    async getMeter(e){
      await ipcRenderer.invoke(eventNames.parametersList,{type:e}).then((res)=>{
        ...
        this.payload_meter.connection_channel) {
          if(this.payload_meter.connection_channel == '1' && 
            ...
            this.payload_meter.port){
              ...
              ipcRenderer.invoke(eventNames.check_port,{ port: this.payload_meter.port }).then((res)=>{
                if(res){
                  this.portError = true
                }
              }).catch(async (error)=>{
    ...
    async submit(){
      ...
      this.payload_meter.token = sessionStorage.getItem("token")
      this.payload_meter.parameters = this.indicators
      const data = JSON.stringify({...this.payload_meter,meter_form:"meter"})
      let requestName = eventNames.createMeter;
      if(this.active?._id){
        requestName = eventNames.updateMeter
      }
      if((this.payload_meter.connection_channel == '0')||(this.payload_meter.connection_channel == '2')){
        ...
        requestName = eventNames.createComMeter;
        if(this.active?._id){
          requestName = eventNames.updateComMeter
        }
      }
      if(this.payload_meter.connection_channel == '1'){
      ...
      await ipcRenderer.invoke(requestName, JSON.parse(data)).then((res) => {
        if(res){
          this.modalHide()
  ...
  async mounted(){
    this.loading = true
    this.comlist = []
    await ipcRenderer.invoke(eventNames.portList).then((res)=>{
      if(res?.data){
        res?.data?.forEach((el)=>{
          if(el.path != 'COM1'){
            this.comlist.push({name:el.friendlyName,classificator:el.path})
      ...
      if(res?.data){
        ...
        Object.keys(res?.data).map(el=>{
          if(el == this.active?.meter_type){
            this.options.push({name:res?.data[el],classificator:el,default: 'active'})
          }else{
            this.options.push({name:res?.data[el],classificator:el})
          }
        })
      }
    }).catch(async (error)=>{
