...
export default {
  ...
  methods: {
    ...
    async submit() {
      ...
      if(this.user){
        ...
        await ipcRenderer.invoke(eventNames.updateAdmin, payload).then((res)=>{
        ...
        let payload = JSON.parse(JSON.stringify(data))
        await ipcRenderer.invoke(eventNames.addAdmin, payload).then((res)=>{
          res && res.status < 300 ? this.modalHide() : ''
        }).catch(async (error)=>{
  ...
  async mounted() {
    ...
    await ipcRenderer.invoke(eventNames.getElectFactories).then((res)=>{
      if(res?.data){
    ...
    await ipcRenderer.invoke(eventNames.getReportOnlyMain).then((res)=>{
      if(res?.data){
        let data = JSON.parse(res?.data)
        this.calcobjs = []
