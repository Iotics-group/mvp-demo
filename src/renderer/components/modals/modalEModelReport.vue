...
export default {
  ...
  methods: {
    async submit(){
      ...
      if(!this.edit){
        ...
        let requestName = eventNames.createCalculationFolder
        ...
        let data = JSON.stringify({...this.payload})
        let requestName = eventNames.updateCalculationFolder
        await ipcRenderer.invoke(requestName, JSON.parse(data)).then((res) => {
          if(res) {
            this.modalHide()
            this.$emit('response',res)
  ...
  async mounted(){
    ...
    if(this.edit && this.idx){
      await ipcRenderer.invoke(eventNames.getSingleObjectEventReport, {id: this.idx,type:this?.objType}).then((res) => {
