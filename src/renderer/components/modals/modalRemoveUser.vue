...
export default {
  ...
  methods: {
    async submit() {
      let response = null
      if(!this.status){
        response = await ipcRenderer.invoke(eventNames.activeAdmin, { id: this.user?._id,token: sessionStorage.getItem('token')})
      }else{
        response = await ipcRenderer.invoke(eventNames.deleteAdmin, { id: this.user?._id,token: sessionStorage.getItem('token')})
      }
      let status = 'inactive'
