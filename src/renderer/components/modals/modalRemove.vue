  ...
  methods: {
    async submit() {
      if(this.data?.objType){
        ...
        let requestName = this.data?.objType === 'meter' ? eventNames.deleteMeter:eventNames.deleteFolder;
        await ipcRenderer.invoke(requestName, { id: this.meter_id, token: sessionStorage.getItem("token") }).then((res) => {
          if(res) {
            this.modalHide()
        ...
        let requestName = this.report === true ? eventNames.deleteCalculationObject:eventNames.deleteElectObject;
        await ipcRenderer.invoke(requestName, { id: this.idx, token: sessionStorage.getItem("token") }).then((res) => {
          if(res) {
            this.modalHide()
            this.$emit('response',res)
          }
