...
export default {
  ...
  methods: {
    ...
    async submit(){
      this.payload.token = sessionStorage.getItem("token")
      ...
      if(!this.edit){
        if((this.payload.type == 'meter' && this.payload.name && this.payload.meter_id)||(this.payload.type != 'meter')){
            ...
            let requestName = this.payload.type == 'meter' ? eventNames.createElectMeter:eventNames.createElectFolder
          ...
          if(this.payload.name && this.id){
            ...
            let requestName = this.payload.type == 'meter' ? eventNames.updateElectMeter:eventNames.updateElectFolder
  ...
  async mounted(){
    ...
    if(this.edit && this.idx){
      await ipcRenderer.invoke(eventNames.getSingleObjectEvent, {id: this.idx,type:this?.objType}).then((res) => {
    ...
    await ipcRenderer.invoke(eventNames.listUseMeterElect).then((res) => {
      if(res.data) {
