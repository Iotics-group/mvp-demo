  ...
  <div class="wrapper">
    ...
    <div class="e-model empanel">
      <div class="model-block" @click="unFocus">
        ...
        <div class="model-btn">
          <button v-if="(user_tools == 'admin')" @click="newObjAdd">{{$locale['newObject'][$i18n.locale.value]}}</button>
        </div>
      </div>
      ...
      <div class="model-block gradient">
        ...
        <div class="model-btn">
          <button v-if="params?.length && (user_tools == 'admin') && (active?.type != 'transformatorvoltage') && (active?.type != 'transformatorelec')" @click="attachParams">{{$locale['saveChanges'][$i18n.locale.value]}} </button>
        </div>
      </div>
    </div>
    ...
    <transition name="modal" :duration="600">
      <modalRemove @response="setstatus($event)" :idx="idx" @modalShow="modalShow" v-if="remove"/>
    </transition>
...
import modalEModel from '../components/modals/modalEModel.vue';
...
export default {
  ...
  methods: {
    ...
    async attachParams(){
      this.params?.forEach((el,indx)=>{
        delete this.params[indx]._id
        delete this.params[indx].model_type
      })
      let data = JSON.stringify({
        id: this.active._id,
        parameters:this.params
      })
      if((this.active.type == 'feeder')){
        this.wrong = true
        this.wrongM = this.$locale['cannotAttachParameterToFeeder'][this.$i18n.locale.value]
        let _t = this
        setTimeout(()=>{
          _t.wrong = false
        },5000)
      }else if((this.active.type == 'meter')){
        this.wrong = true
        this.wrongM = this.$locale['cannotAttachParameterToCounter'][this.$i18n.locale.value]
        let _t = this
        setTimeout(()=>{
          _t.wrong = false
        },5000)
      }else{
        await ipcRenderer.invoke(eventNames.attachParamsElect,JSON.parse(data)).then((res) => {
  ...
  async mounted() {
    ...
    ipcRenderer.on('context-menu-add-obj', async () => {
      this.edit = false
      if(this.idx){
        ipcRenderer.invoke(eventNames.getSingleObjectEvent,{id:this.idx,type:this.objType}).then((res) => {
          if(res?.args){
            let data = JSON.parse(res?.args)
    ...
    await ipcRenderer.invoke(eventNames.metersList).then((res)=>{
      if(res?.data){
