...
export default {
  ...
  methods: {
    ...
    async getEmit(e){
      ...
      this.fetchData = null
      let requestName = this.isReportObject === true ? eventNames.getDashboardEventReport:eventNames.getDashboardEvent
      await ipcRenderer.invoke(requestName,{id:e,query:{...this.filter}}).then((res) => {
      ...
      this.saveMe = false
      const _t = this
      let requestName2 = this.isReportObject === true ? eventNames.realTimeReport:eventNames.realTime
      await ipcRenderer.invoke(requestName2,{id:_t.id,query:{...this.filter}}).then((resrealtime)=>{
        clearInterval(this.interval)
        if(resrealtime?.args){
          _t.realtime = resrealtime.args ? JSON.parse(resrealtime.args) : null;
          this.interval = setInterval(function() {
            ipcRenderer.invoke(requestName2,{id:_t.id}).then((resrealtime)=>{
              _t.realtime = resrealtime.args ? JSON.parse(resrealtime.args) : null;
            })
          },30000)
        }
