  ...
  <div class="wrapper">
    ...
    <div class="e-model empanel">
      ...
      <div class="model-block gradient">
        ...
        <div class="model-btn">
          <button v-if="params?.length && (user_tools == 'admin')" @click="attachParams">{{$locale['saveChanges'][$i18n.locale.value]}}</button>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    async attachParams(){
      ...
      ipcRenderer.invoke(eventNames.attachParamsCalculation,JSON.parse(data)).then((res) => {
