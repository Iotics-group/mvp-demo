...
export default {
  ...
  methods: {
    ...
    if (e) {
      this.id = e
      this.loading = true
      let requestName = this.isReportObject === true ? eventNames.getGraphsAndObjectReportArchive:eventNames.getGraphsAndObjectEventArchive
      let requestName2 = this.isReportObject === true ? eventNames.getGraphsAndObjectReportCurrent:eventNames.getGraphsAndObjectEventCurrent
      this.chartOption = []
      ...
      await ipcRenderer.invoke(requestName, {id: e, query: {archive: {...this.filter}}}).then((res) => {
        if(res?.args){
          this.times = {...this.times,...JSON.parse(res.args.archiveResult)}
          let chartAP = []
          let chartAM = []
          let chartRP = []
          let chartRM = []
          let times = []
          res.args && Object.keys(this.times).map((k,index)=>{
            times[index] = k
      ...
      await ipcRenderer.invoke(requestName2, {id: e, query: {current: {...this.filter2}}}).then((res) => {
        this.chartCurrently = []
        this.chartCurrently2 = []
        if(res?.args){
          this.times2 = {...this.times2,...JSON.parse(res.args.currentResult)}
          let voltage_A = []
          let voltage_B = []
          let voltage_C = []
          let voltage_AB = []
          ...
          let frequency = []
          let times = []
          res?.args && Object.keys(this.times2).map((k,index)=>{
            times[index] = k
  ...
  async mounted() {
    ...
    this.beforeFLB()
    this.beforeFLB2()
    ipcRenderer.invoke(eventNames.getElectricityObjectsEvent,{token: sessionStorage.getItem("token")}).then((res) => {
      this.data = JSON.parse(res.args)
    }).catch((err)=>{
      console.log(err);
    })
    ipcRenderer.invoke(eventNames.getElectricityObjectsEventReport,{token: sessionStorage.getItem("token")}).then((res) => {
