    ...
    methods: {
      ...
      async setdate(e){
        if(e){
          ...
          delete params.id
          await ipcRenderer.invoke(eventNames.getVectorDiagram,{...this.params}).then((res) => {
            this.fetchData = (JSON.parse(res.args))
      ...
      async getEmit(e,bln){
        ...
        await ipcRenderer.invoke(eventNames.getVectorDiagram,{...this.params,limit:this.limit}).then((res) => {
          if(bln){
            this.fetchData = (JSON.parse(res.args))
          }else{
            this.fetchData = {...this.fetchData,...(JSON.parse(res.args))}
          }
          this.finish = res?.data?.finish
        }).catch(async (error)=>{
          console.log(error);
