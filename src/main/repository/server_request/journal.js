...
module.exports.journalRepository = () => {
  ...
  async function insert(args) {
    try {
      if (args.request_type != 'connect') {
        ms.sendMessage({ id: String(args.meter), status: args.status, where: args.request_type })
      }
  ...
  async function update(args) {
    try {
      ...
      if (args.request_type != 'connect') ms.sendMessage({ id: String(args.meter), status: args.status, where: args.request_type })
      await repo.repositories().lastJoinRepository().update(args.meter, updateLastJoin)
